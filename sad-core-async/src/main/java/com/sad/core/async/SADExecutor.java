...
public class SADExecutor implements Executor {
    private static final String TAG = SADExecutor.class.getSimpleName();
    /**
     * debug mode turn
     */
    ...
    public SADExecutor(int coreSize, int queueSize) {
    ...
    protected synchronized void initThreadPool() {
        if (debug) {
            Log.v(TAG, "SADExecutor core-queue size: " + coreSize + " - " + queueSize
    ...
    public static void setThreadPool(ThreadPoolExecutor threadPool) {
        SADExecutor.threadPool = threadPool;
    }
    ...
    /**
     * When {@link #execute(Runnable)} is called, {@link SADExecutor} perform actions:
     * <ol>
    ...
    public void execute(final Runnable command) {
        ...
        synchronized (lock) {
            ...
            if (runningList.size() < coreSize) {
                ...
                //Log.v(TAG, "SADExecutor task execute");
            } else if (waitingList.size() < queueSize) {
                waitingList.addLast(scheduler);
                //Log.v(TAG, "SADExecutor task waiting");
            } else {
                //if (debug) {
                //    Log.w(TAG, "SADExecutor overload , policy is: " + overloadPolicy);
                //}
                switch (overloadPolicy) {
                    ...
                    case DiscardCurrentTask:
                        break;
                    case ThrowExecption:
                        throw new RuntimeException("SADTaskRunnable rejected from lite smart executor. " + command.toString());
                    default:
                        break;
                }
            }
        ...
        if (callerRun) {
            if (debug) {
                Log.i(TAG, "SADExecutor task running in caller thread");
            }
            command.run();
        }
    }
    ...
    private void scheduleNext(WrappedRunnable scheduler) {
        synchronized (lock) {
            ...
            if (!suc) {
                runningList.clear();
                Log.e(TAG,
                        "SADExecutor scheduler remove failed, so clear all(running list) to avoid unpreditable error : " + scheduler);
            }
            if (waitingList.size() > 0) {
                ...
                if (waitingRun != null) {
                    ...
                    Log.e(TAG,
                            "SADExecutor get a NULL task from waiting queue: " + Thread.currentThread().getName());
                }
            } else {
                if (debug) {
                    Log.v(TAG, "SADExecutor: all tasks is completed. current thread: " +
     ...
     * @return this
     */
    public SADExecutor setCoreSize(int coreSize) {
        if (coreSize <= 0) {
     ...
     * @return this
     */
    public SADExecutor setQueueSize(int queueSize) {
        if (queueSize < 0) {
